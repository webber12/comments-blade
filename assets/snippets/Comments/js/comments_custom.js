    ...
    Comments.prototype = {
        ...
        init: function () {
            ...
            $(self._options.commentsWrapper).on('click', self._options.replyBtn, function (e) {
                ...
                $('#comment-' + commentId + '>.comment-wrap').append(form);
                self.initForm(form);
        ...
        cancelReply: function () {
            ...
            } else if (self._options.formPosition === 'prepend') {
                form.insertBefore(self._options.commentsWrapper);
            } else {
                $(document).find("#" + self._options.formPosition).html(form);
            }
            self.initForm(form);
        },
        ...
        initForm: function (form) {
            ...
            $(document).trigger('comments-init-form-success', [ form ]);
                    ...
                    if (response.count) {
                        ...
                        Object.keys(response.comments).forEach(function (id) {
                            ...
                            if (parseInt(id) === 0) {
                                ...
                                $(document).trigger("comments-add-comment-success", [comment]);
                            } else {
                                var element = $('#comment-' + id);
                                element.removeClass(self._options.editableClass);
                                if(element.length > 0) {
                                    element.find("." + self._options.replyCntClass + ":first").text(parseInt(element.find("." + self._options.replyCntClass + ":first").text()) + 1);
                                    $(element).append(comment);
                                    $(document).trigger("comments-add-reply-success", [id, comment]);
                                } else {
                                    $(self._options.commentsWrapper).append(comment);
                                    $(document).trigger("comments-add-comment-success", [comment]);
                                }
            ...
            $.post(self._options.connector,
                ...
                function (response) {
                    if (response.status) {
                        ...
                        $(document).trigger("comments-rate-success", [commentId, response.like, response.dislike, response.count, response.status]);
                    }
